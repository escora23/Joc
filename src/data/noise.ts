// FRONT ULTRA — deterministic gradient noise (owner: data). Worker-safe, allocation-free per call.
// 3D simplex noise (after Gustavson's reference formulation) with a seeded permutation table, plus fractal helpers.
// Output range ~[-1, 1].
...
export class Simplex3 {
  ...
  noise(xin: number, yin: number, zin: number): number {
    const perm = this.perm, pm = this.permMod12, g = GRAD3;
    ...
    if (t0 > 0) {
      ...
      t0 *= t0;
      n += t0 * t0 * (g[gi] * x0 + g[gi + 1] * y0 + g[gi + 2] * z0);
    }
    let t1 = 0.6 - x1 * x1 - y1 * y1 - z1 * z1;
    if (t1 > 0) {
      const gi = pm[ii + i1 + perm[jj + j1 + perm[kk + k1]]] * 3;
      t1 *= t1;
      n += t1 * t1 * (g[gi] * x1 + g[gi + 1] * y1 + g[gi + 2] * z1);
    }
    let t2 = 0.6 - x2 * x2 - y2 * y2 - z2 * z2;
    if (t2 > 0) {
      const gi = pm[ii + i2 + perm[jj + j2 + perm[kk + k2]]] * 3;
      t2 *= t2;
      n += t2 * t2 * (g[gi] * x2 + g[gi + 1] * y2 + g[gi + 2] * z2);
    }
    let t3 = 0.6 - x3 * x3 - y3 * y3 - z3 * z3;
    if (t3 > 0) {
      const gi = pm[ii + 1 + perm[jj + 1 + perm[kk + 1]]] * 3;
      t3 *= t3;
      n += t3 * t3 * (g[gi] * x3 + g[gi + 1] * y3 + g[gi + 2] * z3);
    }
    return 32 * n;
  }
}
