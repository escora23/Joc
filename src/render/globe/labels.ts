// FRONT ULTRA — nation labels on the globe (owner: globe).
// Name + troop count at the sim's label anchor (largest interior area), sized by the nation's on-screen
// extent, clamped to a readable pixel range, faded by zoom / horizon / overlap. One instanced draw call:
// per-glyph instances (em-space quads + atlas uv) reference a per-label row in a small float DataTexture
// (anchor, pixel size, color, alpha) updated every frame; glyph layout is rebuilt only when texts change.
...
attribute vec4 aUv;
attribute vec2 aMeta; // label index, line (0 name, 1 number)
...
void main() {
  ...
  float ow = 0.15 + 0.08 * (1.0 - clamp((vPx - 13.0) / 14.0, 0.0, 1.0));
  float edge = smoothstep(0.5 - ow - w, 0.5 - ow + w, d);
  float halo = smoothstep(0.08, 0.5, d);
  vec3 nat = vColor.rgb;
  vec3 textCol = vLine < 0.5 ? mix(vec3(1.0, 0.98, 0.94), nat, 0.12) * 1.15 : mix(vec3(1.0), nat, 0.45) * 1.05;
  vec3 outline = nat * 0.06;
  vec3 c = mix(outline, textCol, fill);
  float a = max(fill, edge * 0.92);
  // Soft dark shadow + a faint nation-colored glow behind the text.
  vec3 glow = nat * 0.35;
  float ga = halo * 0.45;
  vec3 outc = mix(glow * ga, c, a);
  float outa = max(a, ga);
...
const byPriority = (a: LabelState, b: LabelState): number => (b.visible ? b.prio : -1) - (a.visible ? a.prio : -1);

interface LabelState {
  id: number;
  text: string;
  num: string;
  /** Layout width in em of the name line / number line. */
  wName: number;
  wNum: number;
  alpha: number;
  target: number;
  slot: number;
  // Screen placement (this frame).
  sx: number;
  sy: number;
  px: number;
  prio: number;
...
export function createNationLabels(ctx: GameContext, font: SdfFont | null): NationLabels {
  ...
  const placed: number[] = []; // x0, y0, x1, y1 quadruples
  let textDirty = true;
  let textTimer = 0;
  let nextSlot = 0;
  ...
  function rebuildGlyphs(): void {
    ...
    const emit = (s: string, scale: number, y: number, width: number, slot: number, line: number) => {
      let x = -width / 2;
      for (const ch of s) {
    ...
    for (const s of states.values()) {
      ...
      emit(s.text, 1, 0.08, s.wName, s.slot, 0);
      emit(s.num, NUM_SCALE, -0.78, s.wNum, s.slot, 1);
    }
    geo.instanceCount = n;
  ...
  function syncTexts(): boolean {
    ...
    for (const p of view.playerList) {
      ...
      if (!s) {
        ...
        s = { id: p.id, text: '', num: '', wName: 0, wNum: 0, alpha: 0, target: 0, slot, sx: 0, sy: 0, px: 0, prio: 0, visible: false };
        states.set(p.id, s);
        order.push(s);
        changed = true;
      }
      const text = playerName(p, world).toLocaleUpperCase();
      const num = formatCompact(Math.max(0, Math.round(p.troops)));
      if (text !== s.text || num !== s.num) {
        s.text = text;
        s.num = num;
        s.wName = layoutWidth(text, 1);
        s.wNum = layoutWidth(num, NUM_SCALE);
        changed = true;
      }
    }
    return changed;
  ...
  return {
    ...
    update(dt, opacity) {
      ...
      for (const s of order) {
        ...
        // Horizon test: visible if the camera is above the tangent plane at the anchor.
        const facing = (v.x * cam.position.x + v.y * cam.position.y + v.z * cam.position.z) / v.length() - v.length();
        const horizonFade = Math.min(1, Math.max(0, facing / (0.04 + camDist * 0.02)));
        ...
        const minPx = isHuman ? 21 : 19;
        const px = Math.min(isHuman ? 42 : 36, Math.max(minPx, raw));
        const sizeFade = isHuman ? 1 : Math.min(1, Math.max(0, (raw - 6.5) / 4));
        ...
        s.px = px;
        s.prio = (isHuman ? 1e6 : 0) + px * 100 + Math.sqrt(p.tiles);
        s.visible = true;
        s.target = horizonFade * sizeFade * zoomFade;
      }
      // Greedy overlap removal, most important first.
      order.sort(byPriority);
      placed.length = 0;
      for (const s of order) {
        if (!s.visible || s.target <= 0) continue;
        const w = Math.max(s.wName, s.wNum) * s.px * 0.5 + 4;
        const x0 = s.sx - w, x1 = s.sx + w;
        const y0 = s.sy - s.px * 1.0 - 3, y1 = s.sy + s.px * 1.2 + 3;
        let hit = false;
      ...
      for (const s of order) {
        s.alpha = damp(s.alpha, s.target * opacity, 9, dt);
        if (s.alpha < 0.003 && s.target === 0) s.alpha = 0;
        const p = view.players[s.id];
        if (p && s.slot >= 0) {
          tileXYToLatLon(p.labelX, p.labelY, ll);
          latLonToVec3(ll.lat, ll.lon, ctx.globe.surfaceRadiusAt(ll.lat, ll.lon) + 0.0006, v);
