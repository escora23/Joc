...
import type {
  AllianceRequestView, AllianceView, AttackView, BuildableUnit, CommandKind, EmoteId, GameConfig, GameOverReason,
  GamePhase, GameSpeed, PlayerKind, Personality, PlayerStatsCounters, ScarView, StructureType, StructureView,
  UnitType, WeaponType, WorldEventKind, WorldEventView, WorldInit,
} from './types';
...
export type PlayerCommand =
   ...
   *  tile: the clicked tile (used to bias the push direction). */
...
export type SimEvent =
  ...
  | { type: 'attackStarted'; tick: number; attackId: number; attacker: number; defender: number; troops: number; tile: number; naval: boolean }
...
export function tickUpdateTransferables(u: TickUpdate): Transferable[] {
  ...
  if (u.fronts) for (const f of u.fronts) list.push(f.samples.buffer as ArrayBuffer);
  return list;
}
