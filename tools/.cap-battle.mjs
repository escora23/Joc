...
for (const shot of shots) {
  const T0=Date.now(); const page = await browser.newPage({ viewport: { width: w, height: h } });
  const logs = [];
  page.on('console', (m) => { console.log(((Date.now()-T0)/1000).toFixed(1), m.type(), m.text().slice(0,300)); if(0) logs.push(`[${m.type()}] ${m.text()}`); });
  ...
  try { await page.waitForFunction(() => (window).__shotReady === true || (window).__ready === true, null, { timeout: Number(args.timeout||240000) }); } catch { logs.push('[capture] timeout waiting for __shotReady'); }
